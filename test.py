...
trained_model = torch.load('{}.pt'.format(MODEL))  # train_20190316_1 에서 저장한 가중치 파일 mlp.pt를 불러옴
...
model.load_state_dict(state_dict)  # MLP 클래스의 모델 구조를 가진 mlp 객체에 state_dict의 가중치&바이어스를 입력함
...
for data in data_loader:  # data 객체로 data_loader의 성분을 불러옴
    ...
    classification_results = model(input)  # mlp.pt의 가중치를 입력받은 MLP 클래스의 인스턴스 mlp 에 인풋을 입력
